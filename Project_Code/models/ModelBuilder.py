...
class ModelBuilder(object):
    ...
    __STORED_DATA = '../../../Datasets/ignis-1M.csv'
    ...
    def insert_password(self, pwd: str):
        # Calculate probability for the new password
        prob = self.__markov.ngram_prob(pwd)
        if prob <= 0:
            prob = self.__min_prob
            
        # Create a DataFrame with only the password and its probability
        temp = pd.DataFrame([[pwd, prob]], columns=['password', 'probability'])
        
        # Append the new password DataFrame to the existing one
        self.__password_data = pd.concat([self.__password_data, temp], ignore_index=True)
        
        # Optionally, sort and remove duplicates if needed
        self.__password_data.sort_values(['probability'], inplace=True, ascending=False)
        self.__password_data.drop_duplicates(subset=['password'], keep='last', inplace=True)

        # You may also want to reset the index after appending
        self.__password_data.reset_index(drop=True, inplace=True)
